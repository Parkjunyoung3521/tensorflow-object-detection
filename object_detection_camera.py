...
# GPU의 동적 메모리 할당( Dynamic Memory Allocation )을 활성화시키는 코드.
...
def detect_fn(image):
    ...
    return detection, prediction_dict, tf.reshape(shapes,[-1])

# 라벨 맵 데이터 로딩( Load label map data )
...
while True :
    ...
    # mscoco_label_map.pbtxt 파일을 보면, id 가 1 부터 시작하니까
    # offset 을 1로 만들어 준다.
    label_id_offset = 1
